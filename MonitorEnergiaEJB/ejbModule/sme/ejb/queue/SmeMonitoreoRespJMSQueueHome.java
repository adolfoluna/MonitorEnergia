...
public class SmeMonitoreoRespJMSQueueHome implements SmeMonitoreoJMSQueueRemote {
	...
	private static final Log log = LogFactory.getLog(SmeMonitoreoRespJMSQueueHome.class);
	
	public SmeMonitoreoRespJMSQueueHome() {
	...
	public void write(Serializable object,int priority) {
		try {
			log.info("enviando mensaje a sme_monitoreo_respuestas.....");
			context.createProducer().setPriority(priority).send(jmsqueue, context.createObjectMessage(object));
			log.info("mensaje a sme_monitoreo_respuestas exitosamente enviado....");
		}catch(Exception ex) {
